...
The Calendar Runtime Assembly is now implemented at Prototype maturity. Its thin boundary performs a fresh read, renders current calendar context plus inline proposals and the reinforced approval queue, and delegates durable decisions and execution to the existing Tested workflow. Eight focused tests and **94 repository tests** pass. The combined synthetic acceptance verifies read-to-queue composition, edit/defer/approval behavior, verified execution, restart restoration, and zero live calendar mutations. Canonical CI and review remain before Tested promotion; this is not the full Briefing Engine.
