...
| Pilot Runtime SQLite Durability | Prevent empty-store substitution and provide validated backup/restore for the single-runtime pilot | Mission Control marked-volume contract + Python `sqlite3` online backup API + thin S3-compatible adapter | Prototype | 24 focused durability/offsite/guardian tests; full suite passes 65 tests; separate-process bootstrap, backup, loss, fail-loud startup, restore, and semantic verification pass with zero live mutations; canonical PR #11 CI passed | Railway/R2 deployment code is ready; actual Railway volume encryption evidence, provisioned R2 upload/read-back, provider retention controls, and deployed clean-restore rehearsal remain required before Tested |
