...
| Pilot Runtime SQLite Durability | Prevent empty-store substitution and provide validated backup/restore for the single-runtime pilot | Mission Control marked-volume contract + Python `sqlite3` online backup API | Prototype | 14 focused tests; full suite passes 55 tests; separate-process bootstrap, backup, loss, fail-loud startup, restore, and semantic verification passed with zero live mutations; canonical CI run #10 passed | Host-neutral controls pass locally and in canonical CI; actual encrypted cloud volume, independent backup storage, scheduler/retention, and deployed restore rehearsal remain required before Tested |
